...
def bfs_recursive(root, queue, color_list=[], visited=None):
    ...
    if root_id not in visited:
        ...
        if current_root.left is not None:
            queue.append(current_root.left.id)
        if current_root.right is not None:
            queue.append(current_root.right.id)
    # Рекурсивний виклик функції з тією ж чергою та множиною відвіданих вершин
    return bfs_recursive(root, queue, color_list, visited)
